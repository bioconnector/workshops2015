...
rowSums(mycounts)
which.max(rowSums(mycounts))
topGene <- which.max(rowSums(mycounts))
topGene
mycounts[topGene, ]
...
```{r, echo=FALSE, include=FALSE}
subset(mc2, ctlmean>0 & uvbmean>0)
subset(mc2, ctlmean*uvbmean>0)
```
...
```{r plot_means, echo=FALSE, include=FALSE}
